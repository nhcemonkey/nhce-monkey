...
import bluemonkey.Client.DownloadListener;
...
public class VersionPanel extends JPanel {
	...
	public static final String SUB_NAME = "Lange Monkey";
	public static final String VERSION = "1.4b";// Often Modified
	public static final String SOFT_HOST = "http://code.google.com/p/lange-monkey/";
	...
	public VersionPanel() {
		...
		about.setBackground(BlueMonkey.bgColor);
		// about.setLineWrap(true);
		// about.setWrapStyleWord(true);
		about.setEditable(false);
		rightPanel.add(new JScrollPane(about));
		logoBtn = new JButton("<html>" + SUB_NAME + "<br>当前版本：V " + VERSION
				+ "<br><font color=\"blue\">单击检测新版<font></html>", BlueMonkey.getIconRes("logo.png", 128));
		JPanel logoBtnWrap = new JPanel(new BorderLayout());
		logoBtnWrap.add(logoBtn);
		JButton reportBtn = new JButton("提交Bug或建议", BlueMonkey.getIconRes("report.png", 25));
		...
		final JButton licBtn = new JButton("查看许可", BlueMonkey.getIconRes("license.png", 25));
			...
			public void hyperlinkUpdate(HyperlinkEvent e) {
				if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					if (e.getURL().getProtocol().equalsIgnoreCase("mailto")) {
						if (JOptionPane.showConfirmDialog(BlueMonkey.bm, "是否启动系统默认邮件客户端？", "请选择",
	...
	public static String getMainVersion() {
		...
		try {
			final String html = BlueMonkey.client.get(nhmonkey.VersionPanel.SOFT_HOST);
			...
			public void run() {
				try {
					if (downloading || downloaded)
						return;
					final String html = BlueMonkey.client.get(SOFT_HOST);
					SwingUtilities.invokeLater(new Thread() {
						@Override
						public void run() {
							if (html != null && html.length() > 0) {
								...
								if (wiki != null) {
									...
									version = version.substring(version.indexOf(':') + 1).trim();
									newVersion = version;
									BlueMonkey.output.append("最新版本：" + version);
									if (version.compareToIgnoreCase(VERSION) > 0) {// 有新版
										...
										BlueMonkey.infoPanel.setSelectedIndex(1);
										// --------------Added in
										// 1.2-------------
										...
										// --------------------------------------------
										BlueMonkey.output.append("检测到新版本：" + version + ",建议您及时更新至最新版！");
										BlueMonkey
												.speak("I hava a newer version, please upgrade me as soon as possible!",
														false);
										JOptionPane.showMessageDialog(BlueMonkey.bm, "检测到新版本：" + version
												+ ".\n建议您及时更新至最新版！\n" + info + "（点击Monkey图标开始升级！）", "信息",
												JOptionPane.INFORMATION_MESSAGE);
									} else {
										logoBtn.setText("<html>" + SUB_NAME + "<br>当前版本：V " + VERSION
												+ "<br><font color=\"green\">已是最新版本！<font></html>");
										BlueMonkey.output.append("已是最新版！");
									}
	...
	public static void autoUpdate() {
		...
		if (updateDownLink == null || updateDownLink.length() <= 0) {
			BlueMonkey.output.info("请自行从网站下载新版程序包，并替换现有程序！");
			...
			BlueMonkey.client.clearDownloadListener();
			BlueMonkey.client.addDownloadListener(new DownloadListener() {
				@Override
				...
				public void run() {
					try {
						...
						if (updateGreenVersion)
							...
							fileName += ".exe";
						BlueMonkey.client.downloadSliently(updateDownLink, System.getProperty("user.dir") + '\\',
								fileName, true);
						downloading = false;
						downloaded = true;
						JOptionPane.showMessageDialog(BlueMonkey.bm, "<html>已下载新版程序<font color=\'blue\'>" + fileName
	...
	public static void browseWithDesktop(String url) {
		try {
			...
			if (Desktop.isDesktopSupported() && desktop.isSupported(Desktop.Action.BROWSE)) {
				...
				BlueMonkey.output.warn("不支持系统浏览器调用！");
			}
		} catch (Exception e) {
			BlueMonkey.output.error(e);
		}
	...
	private void logOnGoogle(boolean showHint) throws Exception {
		...
		if (showHint)
			BlueMonkey.output.append("已登陆Google服务器！");
	}
	...
	public void checkLogOnGoogleSuccess(String url) throws Exception {
		...
		BlueMonkey.output.append("首次登陆Google服务器，正在进行初始化操作...");
	...
	public class ReportDialog extends JDialog {
		...
		public ReportDialog() {
			super(BlueMonkey.bm, true);
			setTitle("提交报告");
			setIconImage(BlueMonkey.getIconRes("report.png").getImage());
			setResizable(false);
			setSize(600, 300);
			setLocationRelativeTo(BlueMonkey.bm);
			...
			submitBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					submitReport();
					BlueMonkey.output.info("正在后台提交中,请稍等片刻！（最长可能需要几分钟时间）");
				}
			});
			addWindowListener(new WindowAdapter() {
				@Override
				public void windowOpened(WindowEvent e) {
					BlueMonkey.output.info("提交信息采用SSL加密传送，绝对不会泄露您的身份和隐私，请放心提交！");
				}
			});
		}
			...
			new Thread() {
				public void run() {
					try {
						...
						sb.append("[IP Information: " + getIPInfo() + "]\n");
						sb.append("\n========================= Programe LogArea =========================\n");
						sb.append(BlueMonkey.output.getText());
...
//							return;
						...
						googleClient.addInput("nvp_bu_send", "发送");
						googleClient.post(url);
						BlueMonkey.output.info("提交成功！感谢您的支持！");
					} catch (Exception e) {
						BlueMonkey.output.error(e);
