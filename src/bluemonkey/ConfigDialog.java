...
public class ConfigDialog extends JDialog {
	...
	JCheckBox autoRefreshBox, checkUpdateBox;
	...
	public ConfigDialog() {
		...
		setTitle("设置");
		setIconImage(BlueMonkey.getIconRes("config.png").getImage());
		setResizable(false);
		setSize(450, 500);
		...
		JButton jumpToFillConfigBtn = new JButton("填写设置",BlueMonkey.getIconRes("fill_form.png",30));//Modified in 2.1
		...
		try {
			ConfigInfo info = BlueMonkey.recorder.readConfigInfo();
			if (info != null)
				config.copyConfig(info);
		} catch (IOException e1) {
			BlueMonkey.output.error(e1);
		}
	}
	...
	public void showDialog() {
		...
		if (config.checkUpdate)
			...
			checkUpdateBox.setSelected(false);
	...
	public void save() {
		...
		try {
			...
			setVisible(false);
			BlueMonkey.recorder.writeConfigInfo(config);
		} catch (Exception e) {
			BlueMonkey.output.warn("设置保存失败，请输入正确的信息！");
			config.copyConfig(backup);
		}
		BlueMonkey.output.info("设置保存成功！");
	}
